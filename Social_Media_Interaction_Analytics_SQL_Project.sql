...
-- After Creation Of Tables, Diving Into Insights!...
...
OR location LIKE '%west bengal%';

/*2. Determine the Most Followed Hashtags
Write a query to list the top 5 most-followed hashtags on the platform.*/
SELECT a.hashtag_name, a.hashtag_id, COUNT(*) AS TOP_FIVE
FROM hashtags a
JOIN hashtag_follow b ON a.hashtag_id = b.hashtag_id
GROUP BY a.hashtag_id, a.hashtag_name
ORDER BY TOP_FIVE DESC LIMIT 5;

/*3. Find the Most Used Hashtags
Identify the top 10 most-used hashtags in posts.*/
SELECT a.hashtag_name,
COUNT(b.post_id) AS TOP_10_TAGS_USED
FROM post_tags b
INNER JOIN hashtags a ON b.hashtag_id = a.hashtag_id
GROUP BY a.hashtag_name
ORDER BY TOP_10_TAGS_USED DESC LIMIT 10;

/*4. Identify the Most Inactive User
Write a query to find users who have never made any posts on the platform.*/
SELECT a.user_id, a.username  AS INACTIVE_USERS
FROM users a
LEFT JOIN post b ON a.user_id = b.user_id
...
SELECT a.post_id, COUNT(b.user_id) AS Highest_Likes
FROM post a
RIGHT JOIN post_likes b ON a.post_id = b.post_id
GROUP BY a.post_id
ORDER BY Highest_Likes DESC LIMIT 5;

/*6. Calculate Average Posts per User
Write a query to determine the average number of posts made by users.*/
SELECT 
ROUND(COUNT(a.post_id) / COUNT(DISTINCT b.user_id), 2) 
AS Avg_Posts_Per_User FROM users b
LEFT JOIN post a ON b.user_id = a.user_id;

/*7. Track the Number of Logins per User
Write a query to track the total number of logins made by each user.*/
SELECT a.username, b.user_id,
COUNT(b.login_id) AS Total_Logins
FROM login b
CROSS JOIN
users a ON b.user_id = a.user_id
GROUP BY
b.user_id, a.username
ORDER BY
Total_Logins DESC;
...
GROUP BY a.user_id
HAVING
COUNT(DISTINCT a.post_id) = (SELECT COUNT(*) FROM post);
...
FROM comments b
GROUP BY b.user_id
HAVING COUNT(b.comment_id) = (SELECT COUNT(*) FROM post)) 
AS active_users ON a.user_id = active_users.user_id;

/*11. Identify Users Not Followed by Anyone
Write a query to find users who are not followed by any other users.*/
SELECT a.user_id, a.username, a.email
FROM users a
LEFT JOIN follows b ON a.user_id = b.followee_id
WHERE b.follower_id IS NULL;

/*12. Identify Users Who Are Not Following Anyone
Write a query to find users who are not following anyone.*/
SELECT a.user_id, a.username, a.email
FROM users a
LEFT JOIN follows b ON a.user_id = b.follower_id
WHERE b.followee_id IS NULL;

/*13. Find Users Who Have Posted More than 5 Times
Write a query to find users who have made more than five posts.*/
SELECT a.user_id, a.username, a.email, 
COUNT(b.post_id) AS post_count
FROM users a
RIGHT JOIN post b ON a.user_id = b.user_id
GROUP BY a.user_id, a.username, a.email
HAVING COUNT(b.post_id) > 5;

/*14. Identify Users with More than 40 Followers
Write a query to find users who have more than 40 followers.*/
SELECT a.user_id, a.username, a.email, 
COUNT(b.follower_id) AS follower_count
FROM users a
LEFT JOIN follows b ON a.user_id = b.followee_id
GROUP BY a.user_id, a.username, a.email
HAVING COUNT(b.follower_id) > 40 
...
FROM post ORDER BY caption_length DESC LIMIT 5;
